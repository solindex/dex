...
pub(crate) fn process(program_id: &Pubkey, accounts: &[AccountInfo]) -> ProgramResult {
    ...
    let invoke_accounts = solindex_orderbook::instruction::close_market::Accounts {
    ...
    let invoke_params = solindex_orderbook::instruction::close_market::Params {};

    if let Err(error) = solindex_orderbook::instruction::close_market::process(
        program_id,
        invoke_accounts,
        invoke_params,
    ) {
